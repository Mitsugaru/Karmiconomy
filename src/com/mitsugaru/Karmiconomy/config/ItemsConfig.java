...
public class ItemsConfig implements KConfig
{
			...
			String worldName)
	{
		...
		if(items.containsKey(item))
		{
			...
			final List<String> list = valueFile.getStringList(
					items.get(item).path + ".worlds");
			if (list == null)
			{
				//Check base
				noList = true;
	...
	Map<Item, KCItemInfo> getItemValueMap()
	{
		return items;
	}
		...
		for (final String entry : valueFile.getKeys(false))
		{
			try
			{
				...
				else
				{
					...
					{
						items.put(new Item(key, Byte.parseByte("" + 0),
	...
	private KCItemInfo parseInfo(YamlConfiguration config, String path)
	{
		...
		final boolean iPickupDenyLimit = config.getBoolean(
				...
				plugin.getConfig().getBoolean(
						Field.ITEM_PICKUP.getConfigPath() + ".denyOnLimit",
						false));
		KCItemInfo info = new KCItemInfo(path, iCraftLimit, iCraftPay,
	...
	public class KCItemInfo
	{
		...
		public KCItemInfo(String path, int craftLimit, double craftPay,
