...
public class KarmicEcon {
   ...
   public boolean setupEconomy() {
      ...
      // Check playerpoints
      final Plugin playerPointsPlugin = plugin.getServer().getPluginManager().getPlugin("PlayerPoints");
   ...
   public static boolean pay(Field field, Player player, KConfig config, Item item, String command) {
      boolean paid = false;
      ...
      if(vault && plugin.getPluginConfig().payVault) {
         ...
         if(response != null) {
            ...
            case FAILURE: {
               if(config.getDenyPay(field, item, command)) {
                  ...
                  if(rootConfig.debugEconomy) {
                     plugin.getLogger().severe("Eco Failure: " + response.errorMessage);
                  }
               }
               break;
            }
            case NOT_IMPLEMENTED: {
               ...
               Karmiconomy.sentMessages.put(player.getName(), message);
               if(rootConfig.debugEconomy) {
                  plugin.getLogger().severe("Eco not implemented: " + response.errorMessage);
               }
               break;
            }
            case SUCCESS: {
               if(rootConfig.debugEconomy) {
                  plugin.getLogger().info("Eco success for player '" + player.getName() + "' of amount: " + amount);
               }
               paid = true;
               if(local) {
      ...
      if(playerpoints && plugin.getPluginConfig().payPoints) {
         ...
         if(points == 0) {
            ...
            if(local && paid) {
               ...
               player.sendMessage(LocalString.ECONOMY_FAILURE.parseString(info));
            }
         }
      }
      return paid;
