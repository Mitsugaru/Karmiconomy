...
public class DatabaseHandler{
   ...
   private void checkTables(){
      if(useMySQL){
         ...
         if(!mysql.checkTable(Table.MASTER.getName())){
            plugin.getLogger().info(Karmiconomy.TAG + " Created master table");
            // Create table
         ...
         if(!mysql.checkTable(Table.ITEMS.getName())){
            plugin.getLogger().info(Karmiconomy.TAG + " Created items table");
            mysql.createTable("CREATE TABLE "
                  + Table.ITEMS.getName()
         ...
         if(!mysql.checkTable(Table.COMMAND.getName())){
            plugin.getLogger().info(Karmiconomy.TAG + " Created command table");
            mysql.createTable("CREATE TABLE "
         ...
         if(!mysql.checkTable(Table.DATA.getName())){
            plugin.getLogger().info(Karmiconomy.TAG + " Created data table");
            mysql.createTable("CREATE TABLE "
                  + Table.DATA.getName()
         ...
         if(!mysql.checkTable(Table.PORTAL.getName())){
            plugin.getLogger().info(Karmiconomy.TAG + " Created portal table");
            mysql.createTable("CREATE TABLE "
         ...
         if(!mysql.checkTable(Table.BUCKET.getName())){
            plugin.getLogger().info(Karmiconomy.TAG + " Created bucket table");
            mysql.createTable("CREATE TABLE "
         ...
         if(!mysql.checkTable(Table.MCMMO.getName())){
            plugin.getLogger().info(Karmiconomy.TAG + " Created mcMMO table");
            mysql.createTable("CREATE TABLE "
                  + Table.MCMMO.getName()
         ...
         if(!mysql.checkTable(Table.HEROES.getName())){
            plugin.getLogger().info(Karmiconomy.TAG + " Created Heroes table");
            mysql.createTable("CREATE TABLE "
      ...
      try{
         ...
         if(rs.getResult().next()){
            ...
            do{
               ...
               statement.setString(2, rs.getResult().getString("playername"));
         ...
         rs.closeQuery();
         // import command
         rs = sqlite.select("SELECT * FROM " + Table.COMMAND.getName() + ";");
         if(rs.getResult().next()){
            plugin.getLogger().info("Importing commands table");
            // (row INTEGER PRIMARY KEY, id INTEGER NOT NULL, command TEXT
            // NOT NULL, count INTEGER NOT NULL);");
            PreparedStatement statement = mysql.prepare("INSERT INTO "
                  + Table.COMMAND.getName()
                  + "(id, command, count) VALUES (?,?,?);");
            do{
               statement.setInt(1, rs.getResult().getInt("id"));
               statement.setString(2, rs.getResult().getString("command"));
               statement.setInt(3, rs.getResult().getInt("count"));
               statement.execute();
            }while(rs.getResult().next());
            statement.close();
         }
         ...
         if(rs.getResult().next()){
            ...
            do{
               ...
               statement.setString(4, rs.getResult().getString("partyleave"));
         ...
         if(rs.getResult().next()){
            ...
            do{
               ...
               statement.setInt(10, rs.getResult().getInt("killattackplayer"));
               statement.setInt(11, rs.getResult().getInt("killdefendmob"));
               statement.setInt(12, rs.getResult().getInt("killdefendplayer"));
               statement.setInt(13, rs.getResult().getInt("regainhealth"));
               statement.setInt(14, rs.getResult().getInt("regainmana"));
               statement.setInt(15, rs.getResult().getInt("skillcomplete"));
      ...
      if(!name.contains("'")){
         ...
         if(id == -1){
            ...
            final String query = "INSERT INTO " + Table.MASTER.getName()
                  + " (playername,laston) VALUES('" + name + "','" + laston
                  + "');";
            standardQuery(query);
            // Grab generated id
            id = getPlayerId(name);
   ...
   public boolean checkDateReset(String name){
      ...
      if(valid){
         try{
            ...
            plugin.getLogger().warning("SQL Exception on grabbing player date");
            e.printStackTrace();
   ...
   public boolean resetAllValues(String name){
      ...
      if(drop){
         ...
         standardQuery("DELETE FROM " + Table.ITEMS.getName() + " WHERE id='"
               + id + "';");
         // Drop everything in commands for player id
         standardQuery("DELETE FROM " + Table.COMMAND.getName() + " WHERE id='"
               + id + "';");
      }else{
   ...
   public void incrementData(Field field, String name, Item item, String command){
      boolean inc = false;
      int id = getPlayerId(name);
      if(id != -1){
         inc = true;
      ...
      if(inc){
         ...
         case DATA:{
            ...
            standardQuery("UPDATE " + field.getTable().getName() + " SET "
                  + field.getColumnName() + "='" + value + "' WHERE id='" + id
                  + "';");
            break;
         }
         case ITEMS:{
            ...
            if(item != null){
               if(item.isTool()){
                  standardQuery("UPDATE " + field.getTable().getName()
                        ...
                        + "' WHERE id='" + id + "' AND itemid='" + item.getId()
                        + "';");
            ...
            if(command != null){
               try{
                  PreparedStatement statement = prepare("UPDATE "
                        ...
                        + " SET count=? WHERE id=? AND command=?;");
         ...
         try{
            ...
            case ITEMS:{
               ...
               if(item != null){
                  ...
                  if(item.isTool()){
                     ...
                     query = select("SELECT * FROM "
                           + field.getTable().getName() + " WHERE id='" + id
                           + "' AND itemid='" + item.getId() + "' AND data='"
                           + item.getDurability() + "' AND durability='"
                           + item.getDurability() + "';");
                     if(query.getResult().next()){
                        found = true;
                        data = query.getResult().getInt(field.getColumnName());
                     ...
                     if(!found){
                        ...
                        standardQuery("INSERT INTO "
                              ...
                              + " (id,itemid,data,durability,place,destroy,craft,enchant,playerDrop,pickup) VALUES('"
                              + id + "','" + item.getId() + "','"
                              + item.getData() + "','" + item.getDurability()
                              + "','0','0','0','0','0','0');");
                     }
                  }
               }else{
            ...
            case COMMAND:{
               ...
               if(command != null){
                  ...
                  if(query.getResult().next()){
                     found = true;
                     data = query.getResult().getInt("count");
                  }else{
                     data = 0;
                  }
                  query.closeQuery();
