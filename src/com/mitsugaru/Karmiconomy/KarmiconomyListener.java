...
public class KarmiconomyListener implements Listener {
	...
	public void tameValid(final EntityTameEvent event) {
		...
		if (!event.isCancelled()
			...
			if (event.getOwner() instanceof Player) {
				...
				case WOLF: {
					// Wolf
					if (deny(Field.TAME_WOLF, player, config.tameWolfDenyPay,
				...
				case WOLF: {
					...
					if (!hitLimit(Field.TAME_WOLF, player, null, null)) {
						...
						if (pay(Field.TAME_WOLF, player, null, null)) {
							// Increment
							db.incrementData(Field.TAME_WOLF, player.getName(),
									null, null);
						}
					}
					break;
	...
	public void gameModeValid(final PlayerGameModeChangeEvent event) {
		...
		if (!event.isCancelled()
			...
			case CREATIVE:
			{
				if (deny(Field.CREATIVE, player, config.gameModeCreativeDenyPay,
			...
			case SURVIVAL:
			{
				if (deny(Field.SURVIVAL, player, config.gameModeSurvivalDenyPay,
			...
			case CREATIVE:
			{
				if (!hitLimit(Field.CREATIVE, player, null, null)) {
					...
					if (pay(Field.CREATIVE, player, null, null)) {
						// Increment
						db.incrementData(Field.CREATIVE, player.getName(), null,
								null);
					}
				}
				break;	
			...
			case SURVIVAL:
			{
				if (!hitLimit(Field.SURVIVAL, player, null, null)) {
					...
					if (pay(Field.SURVIVAL, player, null, null)) {
						// Increment
						db.incrementData(Field.SURVIVAL, player.getName(), null,
								null);
					}
				}
				break;
